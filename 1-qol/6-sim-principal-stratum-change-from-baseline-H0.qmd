...
# Load full dataset + "rerandomize" control arm so get OR == 0
...
run_simulation_iteration <- function(iter_num, base_path, full_data, model) {
    ...
    # --- 7. Save the results  if the model ran successfully ---
    if (!is.null(model)) {
...
N_SIMULATIONS <- 1000
OUTPUT_PATH <- "./1-qol/output/sim-principal-stratum-change-from-baseline-H0/" # Base path for all output
N_CORES <- 10 
