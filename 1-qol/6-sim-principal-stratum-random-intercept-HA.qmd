---
Title: Simulation where OR = 0.8 - Random Intercept
---
...
```{r}
run_simulation_iteration <- function(iter_num, base_path, data_path, model_path) {
    # This function runs one full simulation iteration.

    # Load data from disk
    full_data <- read_parquet(data_path)
    ...
    data_for_model <- follow_up |> 
        ...
        mutate(
            ...
            ybaseline = factor(ybaseline, levels = 1:7, ordered = TRUE),
...
N_SIMULATIONS <- 200 
OUTPUT_PATH <- "./1-qol/output/sim-principal-stratum-random-intercept-HA/" # Base path for all output
DATA_PATH <- "./1-qol/sim-data/sim_data_nonpo.parquet" # Path to the data
MODEL_PATH <- "./1-qol/models/model-random-intercept.rds" # Path to the model
N_CORES <- 10
