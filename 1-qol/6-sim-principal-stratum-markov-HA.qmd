---
Title: Simulation where OR = 0.8 - principal stratum
---
    ...
    if (nrow(data_for_model) > 0) {
        model <- tryCatch({
            ...
            blrm(
                ...
                ppo = ~time,
                cppo = function(y) y,
                refresh = 0, # Quiets the STAN output for cleaner logs
                iter = 2000,
                chains = 4,
                method = "sampling",
                seed = 1234 + iter_num,
...
N_SIMULATIONS <- 1000 
OUTPUT_PATH <- "./1-qol/output/sim-principal-stratum-markov-HA/" # Base path for all output
N_CORES <- 10 
...
if (length(completed_files) > 0) {
    completed_iters <- stringr::str_extract(completed_files, "(?<=run_)\\d+") |>
        as.integer() |>
        na.omit() |> # Remove any NAs from parsing failures
        unique()
}
