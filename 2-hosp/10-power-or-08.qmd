...
# Load full dataset
df <- read_parquet(here("2-hosp", "sim-data", "ha_08.parquet")) |> 
...
run_simulation_iteration <- function(iter_num, base_path, full_data) {
    ...
    tx_ids <- sample(unique(full_data$id[full_data$tx == 1]), size = 180, replace = FALSE)
    soc_ids <- sample(unique(full_data$id[full_data$tx == 0]), size = 90, replace = FALSE)
