...
dd <- datadist(df)
options(datadist = "dd")
# Prior on the proportional odds effect (applies to thresholds 1-3)
pcontrast_po <- list(
  c1 = list(tx = 1), 
  c2 = list(tx = 0),
...
# TRUE
predict(model, newdata = df[1,], type = "fitted.ind")[, "Mean"] == apply(predict_blrm_manual(model, newdata = df[1,], include_re = FALSE), c(2, 3), mean)
```
...
estimand_df |> 
  ...
  labs(x = "Difference in mean weeks spent out of hospital and alive (SAT - UC)",
       y = "Density")

ggsave(filename = here("img", "fig-taooh-tau.svg"), width = 8, height = 5)
```
