...
---
We need a new setup to fit transition model with random intercepts for each patient, because it runs on cmdstan not rstan.
## Setup
```{r}
# Load Libraries
library(Hmisc)
...
run_simulation_iteration <- function(iter_num, base_path, full_data) {
    ...
    if (nrow(data_for_model) > 0) {
        ...
        if (!is.null(model)) {
            # Save the full model object (it's too large)
