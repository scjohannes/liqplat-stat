---
Title: Type I Error - Correclty Specified
---
...
run_simulation_iteration <- function(iter_num, base_path, full_data) {
    ...
    if (nrow(data_for_model) > 0) {
        ...
        model <- tryCatch({        
            blrm(
                    y ~ tx + rcs(week, 5) + yprev + ypprev + week %ia% yprev + week %ia% ypprev + ecog_fstcnt + gender + diagnosis + rcs(albumin, 3) + rcs(c_reactive_protein, 3) + rcs(age, 3),
                    data = data_for_model,
                    refresh = 0,
                    iter = 2000,
                    chains = 4,
...
```{r}
N_SIMULATIONS <- 1000
OUTPUT_PATH <- here("2-hosp", "output", "sim-H0-correct-spec")
N_CORES <- 28
