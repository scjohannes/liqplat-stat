...
model_path_brms <- "./2-hosp/models/fit_second_order_brms_cluster.rds"
...
if (REFIT) {
  ...
  if(file.exists(model_path_brms)) {
    fit_brms <- readRDS(model_path_brms)
  } else {
    warning("Model file not found: ", model_path_brms)
    fit_brms <- NULL
  }
}
...
Now we implement this recursive logic in R for `rmsb` and `brms`. We will calculate the unconditional probabilities for each draw separately, resulting in a 3D array where one dimension corresponds to the different draws.
...
results_2 <- sop_markov_second_order(
...
results_2_brms <- sop_markov_second_order(
  model_fit = fit_brms,
  baseline_data = d[1,],
  col_yprev = "yprev",
  col_ypprev = "ypprev",
...
write_SOP <- function(i, tx, baseline_df, path, model){
  ...
  sop_markov_second_order(
    model_fit = model,
    baseline_data = data.frame(pat_id = row$pat_id, tx = tx, ecog_fstcnt = row$ecog_fstcnt, albumin = row$albumin, yprev = row$yprev, ypprev = row$ypprev),
    col_yprev = "yprev",
    col_ypprev = "ypprev",
    col_time = "week",
    times = 1:26,
...
```{r}
path <- "./9-markov-tests/output/second_order_marginalized/"
future::plan(future::multisession, workers=6)
baseline_df <- d |> 
    filter(week == 1) |> 
    select(pat_id, week, ecog_fstcnt, albumin, yprev, ypprev) |> 
    distinct() |> 
    arrange(pat_id)
...
# Control
if (generate_SOPS) {
    furrr::future_walk(1:nrow(baseline_df), \(x) write_SOP(x, 0, baseline_df, path, model = fit_second_order_b_cluster),
                       .progress=TRUE,
                       .options = furrr_options(seed=TRUE))
}
# Treatment
if (generate_SOPS) {
    furrr::future_walk(1:nrow(baseline_df), \(x) write_SOP(x, 1, baseline_df, path, model = fit_second_order_b_cluster),
                       .progress=TRUE,
                       .options = furrr_options(seed=TRUE))
}
```
...
```{r}
files <- fs::dir_ls("./9-markov-tests/output/second_order_marginalized/marginalized_sop_0/", glob = "*.parquet")
...
files <- fs::dir_ls("./9-markov-tests/output/second_order_marginalized/marginalized_sop_1/", glob = "*.parquet")
...
```{r}
d_forward <- d |> 
  # per patient max week is not 26, then fill down with last observation carried forward
  group_by(pat_id) |>
  arrange(week) |>
  mutate(
    max_week = max(week),
  ...
  complete(week = 1:26) |>
  fill(yprev, ypprev, y, tx, .direction = "down")

empirical_sop <- d_forward |>
  group_by(tx, week) |> 
  count(y) |> 
  mutate(
    sop = n / sum(n),
...
plot_empirical <- empirical_sop |> 
  ggplot() +
  aes(x = week, y = sop) +
  geom_line(aes(color = state, linetype = tx), size = 1) +
