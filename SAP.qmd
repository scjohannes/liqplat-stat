...
+:========================+:=============================================================================+
| **Target Population**   | Adult (≥ 18 years) patients with advanced solid cancer eligible for LIQPLAT. |
+-------------------------+------------------------------------------------------------------------------+
...
\beta_{tx} &\sim \text{Normal}(0, 0.55) \\     
\beta_{\text{ecog},j \in 2,3} &\sim \text{Normal}(0, 2.5) \\
\beta_{\text{diag},k \in 2..5} &\sim \text{Normal}(0, 2.5) \\ 
\beta_{crp, m \in 1,2} &\sim \text{Normal}(0, 2.5) \\
\beta_{alb, n \in 1,2} &\sim \text{Normal}(0, 2.5) \\    
...
| Attribute               | Definition                                                                                                                                                                                                                                         |
+:========================+:===================================================================================================================================================================================================================================================+
| **Target Population**   | The Principal Stratum of adult (≥ 18 years) patients with advanced solid cancer eligible for LIQPLAT, defined as those patients who would survive the 26-week follow-up period *irrespective* of their selection status for invitation to the SAT. |
+-------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Treatment Arms**      | (A=1): Selection for invitation to ctDNA-guided care (SAT).                                                                                                                                                                                        |
|                         |                                                                                                                                                                                                                                                    |
|                         | (A=0): No selection for invitation (External Control Group / SoC).                                                                                                                                                                                 |
...
\beta_{t, p \in 1, 2, 3} & \sim \operatorname{Normal}(0, 100) \\
...
-   $\eta_{it}$: Linear predictor for effects assuming proportional odds (PO).
    ...
    -   $\mathbf{X}_i \beta_{\text{covars}}$: Effects of baseline covariates (ECOG status, diagnosis category).[^13]
-   $\gamma_{it,j}$: Models deviation from PO for the effect of time. We allow the effect of time to be non-proportional, constrained to be linear in the outcome category $j$. $\tau_{\text{week}}$ is the non-proportional odds parameter for time.
...
-   **Coefficients (**$\beta_k, \tau_{\text{week}}$): $\text{Normal}(0, 100)$ (Default priors in `rmsb`)
...
5.  **Summarize Posterior:** The collection of $\tau_{W, \text{draw}}^{\text{psATE}}$ values forms the posterior distribution of the primary estimand for QoL. Summarize using posterior median and 95% credible interval.
...
\beta_{\text{tx}} &\sim \text{Normal}(0, 100) \\
\beta_{time, p \in 1,2,3} & \sim \text{Normal}(0, 100) \\
\beta_{\text{yprev1}, k \in 2..4} & \sim \text{Normal}(0, 100) \\
\beta_{\text{yprev2}, l \in 2..4} & \sim \text{Normal}(0, 100) \\
\beta_{t \times yprev1, k \in 2..4} & \sim \text{Normal}(0, 100) \\
\beta_{t \times yprev2, l \in 2..4} & \sim \text{Normal}(0, 100) \\
\beta_{\text{ecog}, j \in 2,3} & \sim \text{Normal}(0, 100) \\
\beta_{\text{diag}, k \in 2..5} & \sim \text{Normal}(0, 100) \\
\beta_{alb, m \in 1,2} & \sim \text{Normal}(0, 100) \\
\beta_{crp, n \in 1,2} & \sim \text{Normal}(0, 100) \\
\gamma_{0i} & \sim \text{Normal}(0, \sigma^2_{\gamma}) \\
\sigma_{\gamma} & \sim \text{Half-Normal}(0, 1)
\end{aligned}
$$
