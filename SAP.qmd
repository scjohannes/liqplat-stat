...
1.  **Generate Posterior Survival Curves:** Using the fitted model (`model_os`), generate posterior draws of the marginalized survival curves for each treatment group ($a=0$ and $a=1$) up to $t_{max}=182$ days.
2.  **Calculate RMST per Draw:** For each posterior draw, calculate the RMST for both treatment groups ($\widehat{\text{RMST}}^0$ and $\widehat{\text{RMST}}^1$) by integrating the respective survival curves from $t=0$ to $t_{max}=182$ days. We will approximate the integral using the trapezoidal rule on 200 discrete time points.
3.  **Calculate RMST Difference per Draw:** For each posterior draw, compute the difference $\widehat{\tau}_{OS, \text{draw}} = \widehat{\text{RMST}}^1_{\text{draw}} - \widehat{\text{RMST}}^0_{\text{draw}}$.
4.  **Summarize Posterior:** The collection of $\widehat{\tau}_{OS, \text{draw}}$ values forms the posterior distribution of the primary estimand. Summarize this distribution using the posterior median and a 95% credible interval.

Example code for obtaining the RMST difference is provided in @sec-appendix-os-code.
...
-   **Treatment Parameter (**$\beta_{tx}$): We use prior predictive simulation to select a prior for $\beta_{tx}$ that implies a clinically plausible distribution for our estimand $\tau_{W}^{\text{psATE}}$. Our simulation, based on historical data, suggests that a prior of $\mathcal{N}(0, 0.5)$ on $\beta_{tx}$ [^14]produces a prior distribution for $\tau_{QoL}^{\text{psATE}}$ with a median at $0$ weeks that has about $80\%$ of its prior mass between $[-3, +4]$.
...
[^14]: In other words, we place 80% of our prior probability mass for the odds ratio of the treatment effect between $0.52$ and $1.90$.
...
4.  **Calculate Difference per Draw:** For each posterior draw, compute the difference $\widehat{\tau}_{W, \text{draw}}^{\text{psATE}} = \widehat{\mathbb{E}}[W^1]_{\text{draw}} - \widehat{\mathbb{E}}[W^0]_{\text{draw}}$.
5.  **Summarize Posterior:** The collection of $\widehat{\tau}_{W, \text{draw}}^{\text{psATE}}$ values forms the posterior distribution of the primary estimand for QoL.
...
2.  **Calculate Weekly SOPs:** For each posterior draw, calculate the SOPs, $\widehat{P}(Y_{t}^a = j)$, for each week $t=1, \dots, 26$, state $j=1, \dots, 5$, and treatment arm $a \in \{0, 1\}$. This requires a recursive calculation accounting for the second-order dependency and marginalizing over baseline covariates, baseline state pairs ($y_{-1}, y_0$), and random effects. Deatils can be found in \[placeholder\].
...
4.  **Calculate Difference per Draw:** For each posterior draw, compute the difference $\widehat{\tau}_{TAOOH, \text{draw}} = \widehat{\mathbb{E}}[W^1]_{\text{draw}} - \widehat{\mathbb{E}}[W^0]_{\text{draw}}$.
5.  **Summarize Posterior:** The collection of $\widehat{\tau}_{TAOOH, \text{draw}}$ values forms the posterior distribution of the primary estimand for TAOOH.
