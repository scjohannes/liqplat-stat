...
![This figure shows the model derived marginal survival curves, with the Kaplan-Meier curves overlaid as dashed lines. The sample ($n=270$) is from simulated data with a true underlying $OR = 0.9$ for transitioning to a worse state for the group selected for invitation to the single-arm trial ($\tau_{OS} \approx 3$ days).](img/fig-os-survcurves.svg){#fig-os-survcurves}

![Posterior distribution of the primary estimand, the expected difference the restricted mean survival time ($\hat{\tau}_{OS}$). The sample ($n=270$) is from simulated data with a true underlying $OR = 0.9$ for transitioning to a worse state for the group selected for invitation to the single-arm trial ($\tau_{OS} \approx 3$ days).](img/fig-os-tau.svg){#fig-os-tau width="500"}
...
We will repeat all primary analyses described above for follow-up periods of 12 months and 24 months. The estimands remain the same, extending the time horizon $t_{max}$ to 365 days (12 months) and 730 days (24 months), respectively.
...
Results will be reported according to CONSORT guidelines where applicable, adapted to the SAT design. Posterior distributions for primary estimands will be visualized (e.g., density plots) and summarized (median, 95% credible interval \[CrI\]).
