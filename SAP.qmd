...
&\quad + \sum_{j=1}^{2} \beta_{mGPS, j} \cdot \text{mGPS}_{ij}
\end{aligned}
$$
...
-   $\text{ecog}_i$ is a binary indicator for ECOG performance status ($0$ if ECOG 0-1, $1$ if ECOG $\geq 2$),
-   $\text{stage}_i$ is a binary indicator for disease stage ($0$ if locally advanced non-operable, $1$ if metastatic),
-   $\text{mGPS}_{ij}$ represents the modified Glasgow Prognostic Score (mGPS) categories, coded as dummy variables for $j \in \{1, 2\}$ (with mGPS=0 as reference).

The modified Glasgow Prognostic Score (mGPS) [@Proctor2011-tc] is a validated prognostic tool based on serum CRP and albumin levels:
...
In LIQPLAT QoL, measured using the European Organisation for Research and Treatment of Cancer (EORTC) QLQ-C30 and QLQ-C15 questionnaires [@Fayers2002-ra].[^3] The QoL scale (question 30) is an ordinal outcome with seven levels, ranging from 1 (very poor) to 7 (excellent), as shown below:
...
**Derived Quantities:** We will report the marginal probability of an error errors.
...
where $i$ indexes patients, $\mathbf{X}_i$ represents histological types of the a patient's cancer with coefficients $\boldsymbol{\beta_1}$.
...
We evaluate the time from treatment initiation to the first occurrence of treatment switch or discontinuation. To isolate the effect of treatment assignment we estimate the cause-specific hazard, i.e., we censor patients who die prior to switching or stopping.

We utilize the same Bayesian proportional hazards model specified for Overall Survival (@sec-os-analysis). The primary estimand is the cause-specific hazard ratio.

### Time to Best Supportive Care

We evaluate the time from first-line treatment initiation to the switch to BSC. To specifically assess the timing of the decision to transition to palliative care (as a proxy for recognizing ineffective therapy) conditional on the patient being alive, we estimate the cause-specific hazard. In this analysis, patients who die without having switched to BSC are censored at the date of death.

We utilize the same Bayesian proportional hazards model as specified for Overall Survival (@sec-os-analysis). We will report the posterior distribution of the cause-specific hazard ratio.
