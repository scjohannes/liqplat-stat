...
We use the same Bayesian proportional hazards model specified for Overall Survival (@sec-os-analysis). The primary estimand is the cause-specific hazard ratio.
...
We use the same Bayesian proportional hazards model as specified for Overall Survival (@sec-os-analysis). We will report the posterior distribution of the cause-specific hazard ratio.
...
# Note: We don't have stage well defined in historic data, so we will use diagnosis (5 level factor) instead for illustrative purposes
md.pattern(df)
