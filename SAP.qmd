...
-   **Treatment Parameter (**$\beta_{tx}$): We use prior predictive simulation to select a prior for $\beta_{tx}$ that implies a clinically plausible distribution for our estimand $\tau_{W}^{\text{psATE}}$. Our simulation, based on historical data, suggests that a prior of $\mathcal{N}(0, 0.5)$ on $\beta_{tx}$ [^14]produces a prior distribution for $\tau_W^{\text{psATE}}$ with a median at $0$ weeks that has about $80\%$ of its prior mass between $[-3, +4]$.
