...
![Patients are randomly selected for invitation to the single-arm trial (SAT) from a prospective research registry. Some eligible participants might not be considered for selection for operational reasons, e.g., inpatients who require emergency treatment before trial staff was made aware of their existence. All outcome data are obtained from routinely collected data. The design enables an unconfounded (randomized) comparison of patients who were selected for invitation to the SAT with patients who were eligible but not selected for invitation.](img/liqplat-flow.png){#fig-trial-design}
...
Secondary endpoints as described in the protocol (e.g., PFS, number of imaging exams) will be analyzed but are considered exploratory and are thus no pre-specified in more detail.
...
#| echo: false
#| message: false
#| tbl-cap: "Operating characteristics for Overall Survival (OS) analysis under various true effect sizes (difference in RMST at 182 days)."
...
::: callout-warning
**Informative Missingness due to Routine Care Data Collection**: A design limitation is that QoL is measured during routine appointments. If the ctDNA strategy influences clinical decisions (e.g., earlier switch to BSC, different follow-up intensity), it affects *both* the patient's true QoL *and* the probability of observing QoL. This creates a potential for informative missingness. To assess the potential for differential informative missingness, we will compare the cumulative incidence of switching to Best Supportive Care (BSC) between the two trial arms. If the rates and timing of switching to BSC are similar between groups, it reduces the concern that missingness driven by palliative care transitions biases the treatment comparison.
:::
...
$$
\begin{align*}
\operatorname{logit}\left( P(y_{it} \ge j \mid y_{it'}, t, \Delta t, \mathbf{x}_i, \text{tx}_i) \right) & = \alpha_j + \eta_{it} + \gamma_{it,j} \\
& \quad \text{for }j = 2, \dots, 7\\
...
-   $\alpha_j$: Category-specific intercepts.
-   $\eta_{it}$: Linear predictor for effects assuming proportional odds (PO).
    ...
    -   $f(\text{t})$: Flexible function of time[^7] modeled as a restricted cubic spline with 4 knots.[^8]
...
-   **Treatment Parameter (**$\beta_{tx}$): We use prior predictive simulation to select a prior for $\beta_{tx}$ that implies a clinically plausible distribution for our estimand $\tau_{W}^{\text{psATE}}$. Our simulation, based on historical data, suggests that a prior of $\mathcal{N}(0, 0.5)$ on $\beta_{tx}$[^14] (i.e., on the log odds scale) produces a prior distribution for $\tau_{QoL}^{\text{psATE}}$ with a median at $0$ weeks that has about $80\%$ of its prior mass between for a increase in the time spent with 'good' or better QoL by $4$ weeks (benefit) and a decrease by $3$ weeks (harm). The prior is placed on the log odds scale and not on the scale of the estimand because of technical limitations.
...
[^13]: Unlike the TAOOH model, we do not include a patient-level random intercept. With an anticipated average of only \~4 observations per patient for this outcome, estimation of a random intercept variance component is expected to be unstable. While this omission risks Type I error inflation if significant unmodelled correlation exists, it is considered a necessary trade-off given the sparse data. The Type I error inflation is mtigated by the skeptical prior.

[^14]: In other words, we place 80% of our prior probability mass for the odds ratio of the treatment effect between $0.53$ and $1.90$.
...
![Marginal state occupancy probabilities (SOP) with 95% credible bands based the model fitted to a sample ($n=216$) from simulated data with a true difference in good or better QoL of $2$ weeks. SOPs have been grouped for illustrative purposes. States 1 to 3 are categorized as 'Good', state 4 as 'Average', and states 5-7 as 'Bad' QoL.](img/qol-primary-estimand-sop-line-grouped.svg){#fig-qol-sops}

![Posterior distribution of the primary estimand, difference in weeks with good or better quality of life ($\tau_{QoL}$), defined as state 1, 2, or 3, based on a sample ($n=216$) from simulated data with a true underlying difference of two weeks. Larger values favour the group invited to the SAT.](img/qol-primary-estimand-posterior.svg){#fig-qol-tau width="500"}

#### Simulated Power

@tbl-oper-char-qol shows frequentist operating characteristics of the transition model. The data-generating mechanism assumes a sample size of $N = 216$, as we assume that $20\%$ of initially $270$ patients would die before 6 months. $85\%$ of post-baseline observations are missing completely at random (MCAR), which equals about $4$ post-baseline measurements per patient. A first order transition model was fit to historical data to obtain plausible QoL trajectories. The data were then generated using a latent Brownian Motion model to approximate the trajectories from the previous step, to create three scenarios with a difference in good QoL states of $0$, $1$ week, $2$ weeks, and $3$ weeks over the 26-week period. For each scenario, 1,000 datasets were simulated and analyzed using the proposed Bayesian first-order Markov ordinal transition model. The power is estimated as the proportion of simulations where the posterior probability of any benefit exceeds 0.95. We will not use the posterior to make a dichotomous claim of benefit. However, for the purpose of evaluating the operating characteristics of our analysis plan, we assess power and Type I error under the hypothetical decision rule of requiring a posterior probability of benefit greater than 0.95.

```{r}
#| label: tbl-oper-char-qol
#| tbl-cap: Bayesian Power and Type I error when $P(\text{benefit}) > 0.95$ is chosen as a cut-off for an assertion of benefit.
#| echo: false
...
summary_table <- sim_summary |>
  mutate(
    effect_size_formatted = sprintf("%.1f", effect_size),
    power_formatted = sprintf("%.1f%% (%.1f%%)", power * 100, power_mcse * 196),
...
$$
\begin{align*}
\operatorname{logit}\left( P(y_{it} \ge j \mid y_{i, t-1}, y_{i, t-2}, t, \mathbf{x}_i, \text{tx}_i) \right) & = \alpha_j + \eta_{it} + \gamma_{0i} \\
& \quad \text{for }j = 2, \dots, 5\\
...
-   $\gamma_{0i}$: A patient-specific random intercept to account for within-patient correlation beyond the Markov dependency.

#### Prior Specifications

We assign a regularizing prior of $\mathcal{N}(0, 0.541)$ to the treatment effect $\beta_{tx}$. We used prior predictive simulation to select this prior, which implies a clinically plausible distribution for our estimand $\tau_{TAOOH}$. Our simulation, based on historical data, suggested that this prior on the log odds scale produces a prior distribution for $\tau_{TAOOH}$ with a median at 0 weeks (no difference), with 80% of its mass on $\beta_{tx}$ between $\log(0.5) \approx -0.693$ and $\log(2.0) \approx +0.693$, which our simulations mapped to an asymmetric 80% prior belief interval for $\tau_{TAOOH}$ of approximately $[-6.0, +3.0]$ weeks.

The intercepts $\alpha_j$ have priors induced by a Dirichlet(0.392) distribution on the baseline cell probabilities, as recommended by the `rmsb` package for 5 outcome levels. All fixed-effect regression coefficients are assigned weakly informative $\mathcal{N}(0, 100)$ priors (default in `rmsb`). The standard deviation of the random intercepts is given a Half-Normal(0, 1) prior (default in `rmsb`)
...
The model will be fitted using the `blrm` function from the `rmsb` package, including the `cluster(id)` term for random effects. MCMC settings as per OS analysis. Example implementation code is provided in @sec-appendix-taooh-code.

#### Obtaining the Estimand

Using the fitted model, we generate posterior draws of the marginalized State Occupancy Probabilities (SOPs) under each treatment ($a=0$ and $a=1$) over the 26-week follow-up period. For each posterior draw, we calculate $\widehat{P}(Y_{t}^a = j)$ for each week $t=1, \dots, 26$, state $j=1, \dots, 5$, and treatment. This requires a recursive calculation accounting for the second-order dependency and marginalizing over baseline covariates, baseline state pairs ($y_{-1}, y_0$), and random effects. Details can be found in \[placeholder\].
...
A sensitivity analysis for the QoL outcome has not yet been finalized. Potential approaches under consideration include varying the definition of "Good" QoL states (e.g., including only states 1-2 rather than 1-3) or exploring alternative handling of missing data mechanisms. This section will be updated prior to database lock.
