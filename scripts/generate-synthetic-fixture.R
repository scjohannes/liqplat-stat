...
synthetic_fixture <- function(root = tempfile("liqplat-synthetic-"),
  ...
  dirs <- file.path(root, c(
    "data/schema", "results/population", "results/os", "artifacts/checkpoints",
  ...
  cohort <- data.frame(
    ...
    follow_up_end_date = as.Date(c("2026-07-01", NA)),
  ...
  survival <- derive_survival_endpoint(
    ...
    follow_up_end_col = "follow_up_end_date", lock_date = as.Date("2026-08-21"),
    horizon_days = 182L
  )
  ...
  # Hash/checkpoint exercise: all files are local synthetic text artifacts.
  checkpoint_input <- file.path(root, "checkpoint-input.txt")
  checkpoint_output <- file.path(root, "checkpoint-output.txt")
  checkpoint_code <- file.path(root, "checkpoint-code.R")
  checkpoint_path <- file.path(root, "artifacts", "checkpoints", "synthetic.yml")
  writeLines("synthetic input", checkpoint_input, useBytes = TRUE)
  writeLines("synthetic output", checkpoint_output, useBytes = TRUE)
  writeLines("synthetic code", checkpoint_code, useBytes = TRUE)
  checkpoint_config <- list(data_lock = "2026-08-21", synthetic = TRUE)
  finalize_checkpoint(
    checkpoint_path, "synthetic", checkpoint_input, checkpoint_output,
    checkpoint_code, checkpoint_config, metadata = list(status = "complete")
  )
  checkpoint_current <- checkpoint_is_current(
    checkpoint_path, "synthetic", checkpoint_input, checkpoint_output,
    checkpoint_code, checkpoint_config
  )
  if (!isTRUE(checkpoint_current)) stop("Synthetic checkpoint did not round-trip.")

  # Make a self-contained report project.  The .here marker ensures
  # here::here() resolves the temporary root during Quarto rendering.
  report_project <- file.path(root, "report-project")
  file.create(file.path(report_project, ".here"))
  report_source <- file.path(project_root, "reports")
  file.copy(file.path(report_source, "analysis-report.qmd"),
            file.path(report_project, "reports"), overwrite = TRUE)
  ...
  list(
    ...
    report_qmd = file.path(report_project, "reports", "analysis-report.qmd")
  )
}
