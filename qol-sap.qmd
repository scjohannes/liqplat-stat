...
Let $y_{it}$ be the ordinal QoL state for patient $i$ at week $t$. The outcome has $K = 7$ ordered categories, where $y_{it} \in \{1, 2, \dots, K\}$ (1=best). Let $y_{it'}$ be the patient's last observed state at a prior week $t'$ where, and let the time gap be $\Delta t = t - t'$.

The model is specified as a cumulative logit model for the transition probabilities. For each threshold $j \in \{2, \dots, K\}$ we model the log-odds that a patient's QoL is in category $j$ or higher (worse):

$$
\begin{align*}
\operatorname{logit}\left( P(y_{it} \ge j | y_{it'}) \right) & = \alpha_j + \eta_{it} + \gamma_{it,j} && \text{for }j = 2, \dots, K\\
...
$$
\begin{align*}
\text{For baseline cell probabilities }(\pi_1, \dots, \pi_K) & \sim \text{Dirichlet}(0.308)\text{, which induces priors on } \alpha_j \\
...
-   In the summations $k$ is an index representing the possible last observed QoL states, ranging from $2$ to $K$. State $k = 1$ servers as the reference category.

-   $\eta_{it}$ is the main linear predictor for effects assumed to satisfy the proportional odds assumption. It includes:

    -   $\beta_{\text{tx}}$: The main effect of the treatment arm.
    -   $f(t)$: A flexible function of study week $t$ (restricted cubic spline).
    -   $\beta_{\text{yprev}=k}$: The effect of the previous QoL state.
    -   $\beta_{\text{gap}}$: The linear effect of the time gap since the last measurement.
    -   $\beta_{\text{yprev}=k \times \text{gap}}$: An interaction term between the last state and the time gap.
    -   $\mathbf{X}_i \beta_{\text{covars}}$: A vector baseline covariate effects, specifically functional status at baseline and diagnosis category.
